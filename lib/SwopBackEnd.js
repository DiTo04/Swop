...
exports.getKommunListFromUserAnswers = function (answerList) {
	...
	var result = {dist:probabilityForUser,
			  kommuner: getListOfJuristictionMatch(probabilityForUser,kommunData)};
	console.log(result)
	return result;
}
...
function getListOfJuristictionMatch(partyProbs, kommunProbs) {
	var result =  kommunProbs.map(function(kommunProb){
						...
						for(party in partyProbs){
							sum += Math.pow(partyProbs[party] - (kommunProb.fordelning.find(function(p){
								if(p.parti == "FP") {
									return "L" == party;
								}
								return p.parti == party}).andel/100), 2)
						}
